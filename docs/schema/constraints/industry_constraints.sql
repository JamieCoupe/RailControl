...
-- Industry track route (subclass of track_route)
ALTER TABLE industry_track_route
ADD CONSTRAINT fk_industry_track_route_track_route
    FOREIGN KEY (id)
    REFERENCES track_route(id)
    ON DELETE CASCADE,
ADD CONSTRAINT chk_industry_track_route_load_time_nonnegative
    CHECK (load_time_minutes IS NULL OR load_time_minutes >= 0);

-- Map: industry ↔ track_route (industry spurs)
ALTER TABLE industry_track_route_map
ADD CONSTRAINT fk_itr_map_industry
    FOREIGN KEY (industry_id)
    REFERENCES industry(id)
    ON DELETE CASCADE,
ADD CONSTRAINT fk_itr_map_track_route
